...
int main() {
    ...
    while (!adres_global->czy_ewakuacja) {
        ...
        int delay = 0;
        if (adres_global->kucharz_speed == 1) delay = -1; // Szybciej
        else if (adres_global->kucharz_speed == 2) delay = 2; // Wolniej
        ...
        if (cooldown <= 0) {
            ...
            for (int j = 0; j < MAX_TASMA; j++) {
                if (adres_global->tasma[j].rodzaj > 0 &&
                    ...
                    adres_global->tasma[j].rezerwacja_dla == 0) {
                    ile_standardowych++;
                }
            }


            int priorytet_standard = (ile_standardowych < 5);

            if (!priorytet_standard) {
                // Jeśli taśma pełna, sprawdź zamówienia specjalne
            ...
            if (typ > 0 && adres_global->tasma[0].rodzaj == 0) {
                ...
                // Jeśli to było specjalne, wyczyść tablet
                if (tablet_idx != -1) {
