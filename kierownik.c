...
void handler(int sig) {
    ...
    if (sig == SIGUSR1) {
        ...
        // SYGNAŁ 1: Przyspiesz 2x wydawanie dań

        if (pid_kucharza > 0) {

            kill(pid_kucharza, sig);

            printf(K_CYAN "[Kierownik] SYGNAŁ 1 - Przyspiesz kuchnie 2x!\n" K_RESET);

        }

    } else if (sig == SIGUSR2) {

        // SYGNAŁ 2: Zmniejsz 50% wydawanie dań

        if (pid_kucharza > 0) {

            kill(pid_kucharza, sig);

            printf(K_CYAN "[Kierownik] SYGNAŁ 2 - Zmniejsz produkcje do 50%%!\n" K_RESET);
